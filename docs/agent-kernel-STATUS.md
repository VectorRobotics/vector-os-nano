...
- **NEXT (resume here): track-debt cleanup** on `feat/playground-vln` (each its own TDD'd, real-sim/unit-verified, green commit) — remaining priority (R16+): (7) go2-trot arrival tuning (capture-creep coasts ~1-4cm past the break on settle → `reached` straddles tol; reliable with ~4cm margin) ← R16; (8) optional `sysnav_runtime.py` extraction; plus queued: auto-re-establish full SysNav pipeline on switch (feed+nodes, rides DQ-16 — R14 ships the warn half only); `route()=None` for un-keyworded switch synonyms (add to `_RULES`); add `DEBUG.md` to `.gitignore`; test-isolation hardening for the cross-test MUJOCO_GL flake. **R17 = next REVIEW round (every 4th).** **DONE:** R10 (429-cap + 2 nav cleanups, 3d60330); R11 (switch-via-NL routing, 6e1f123); R12 (`step_output` NameError + moat guard, fb7abf4); R13 REVIEW (audit + R11 over-reach fix, 2559fcd); R14 (switch SysNav-feed-drop notice, 2ddfaab); R15 (capability-probe test drives real `boot_go2_agent`). **PENDING owner (heavy, gated — do NOT start without go/no-go):** DQ-15 (FAR ROS2 colcon rebuild), DQ-16 (SysNav `.venv-sysnav` re-provision + TRT-engine verify). Also M2-followup: go2 photoreal-VLN (camera 10th-render hang + lidar locate) + VFOV depth↔latitude reconcile (rides DQ-16).
