...
- **NEXT (resume here): track-debt cleanup** on `feat/playground-vln` (each its own TDD'd, real-sim/unit-verified, green commit) — priority: (1) `recognize_navigate` 429-retry has NO cap → burns timeout when qwen-VL is rate-limited (REAL bug, surfaced in c11/M5); (2) switch-via-NL routing (LLM picks raw `bash`/`launch_explore.sh` over `switch_embodiment` — design a workflow first, don't rush the launch path); (3) remove leftover `VECTOR_CMDVEL_LOG` diag in `mujoco_go2.py`; (4) `_nav_controller.route_and_drive` per-leg `timeout` early-break; (5) switch drops active SysNav feed (re-wire/warn); (6) capability-probe test → drive `boot_go2_agent`; (7) go2-trot arrival tuning; (8) optional `sysnav_runtime.py` extraction. **PENDING owner (heavy, gated — do NOT start without go/no-go):** DQ-15 (FAR ROS2 colcon rebuild), DQ-16 (SysNav `.venv-sysnav` re-provision + TRT-engine verify). Also M2-followup: go2 photoreal-VLN (camera 10th-render hang + lidar locate) + VFOV depth↔latitude reconcile (rides DQ-16).
