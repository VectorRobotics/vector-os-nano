...
- **NEXT (resume here): track-debt cleanup** on `feat/playground-vln` (each its own TDD'd, real-sim/unit-verified, green commit) — remaining priority: (2) **switch-via-NL routing** (LLM picks raw `bash`/`launch_explore.sh` over `switch_embodiment` — design a workflow first to assess blast-radius, don't rush the launch path) ← R11; (5) switch drops active SysNav feed (re-wire/warn, rule8); (6) capability-probe test → drive `boot_go2_agent`; (7) go2-trot arrival tuning (capture-creep coasts ~1-4cm past the break on settle → `reached` straddles tol; reliable with ~4cm margin); (8) optional `sysnav_runtime.py` extraction. **DONE (R10, commit pending):** (1) `recognize_navigate` 429-cap, (3) `VECTOR_CMDVEL_LOG` removed, (4) `route_and_drive` per-leg `timeout` early-break. **PENDING owner (heavy, gated — do NOT start without go/no-go):** DQ-15 (FAR ROS2 colcon rebuild), DQ-16 (SysNav `.venv-sysnav` re-provision + TRT-engine verify). Also M2-followup: go2 photoreal-VLN (camera 10th-render hang + lidar locate) + VFOV depth↔latitude reconcile (rides DQ-16).
