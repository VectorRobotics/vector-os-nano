...
- **NEXT (resume here): track-debt cleanup** on `feat/playground-vln` (each its own TDD'd, real-sim/unit-verified, green commit) — remaining priority: (5) switch drops active SysNav feed (re-wire/warn, rule8) ← R12; (6) capability-probe test → drive `boot_go2_agent`; (7) go2-trot arrival tuning (capture-creep coasts ~1-4cm past the break on settle → `reached` straddles tol; reliable with ~4cm margin); (8) optional `sysnav_runtime.py` extraction. New debt surfaced R11: GoalVerifier `name 'step_output' is not defined` runtime error on the VGG walk-verify path (cosmetic — walk still executes; investigate). **DONE:** R10 (1) `recognize_navigate` 429-cap, (3) `VECTOR_CMDVEL_LOG` removed, (4) `route_and_drive` per-leg `timeout` early-break (commit 3d60330); R11 (2) switch-via-NL routing. **PENDING owner (heavy, gated — do NOT start without go/no-go):** DQ-15 (FAR ROS2 colcon rebuild), DQ-16 (SysNav `.venv-sysnav` re-provision + TRT-engine verify). Also M2-followup: go2 photoreal-VLN (camera 10th-render hang + lidar locate) + VFOV depth↔latitude reconcile (rides DQ-16).
